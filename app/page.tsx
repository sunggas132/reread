...
export default function StockDashboard() {
  ...
  // 1. 상태 통합 관리 (realreal 저장소 연동용) [cite: 2026-01-11]
  ...
  // 🎨 티커 색상 동기화 함수: 모든 리스트에서 해당 티커의 색상을 일괄 변경 [cite: 2026-01-11]
  ...
  useEffect(() => {
    const saved = localStorage.getItem('stockApp_vFinal_Sync');
    if (saved) {
  ...
  useEffect(() => {
    if (isLoaded) localStorage.setItem('stockApp_vFinal_Sync', JSON.stringify({ stocks, cash: totalCashUSD, goal: targetPortfolio, watch: watchlist }));
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen relative font-sans text-slate-900 pb-20 overflow-x-hidden">
      {/* 1. MAIN (기존 유지) */}
      {activeTab === 'MAIN' && (
        <div className="p-4 animate-in fade-in duration-500">
          <div className="flex justify-between items-center mb-6">
            <div><h1 className="font-black text-2xl italic uppercase tracking-tighter">My Assets</h1><p className="text-[10px] font-bold text-slate-400 uppercase">₩{exchangeRate} <button onClick={updateRealTimeData}><RefreshCw size={10}/></button></p></div>
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-slate-100 rounded-xl"><Menu size={24}/></button>
          </div>
          <div className="bg-[#0f172a] text-white p-6 rounded-[35px] shadow-2xl mb-8">
            ...
            <div className="flex justify-between items-center mt-6 border-t border-white/10 pt-4 px-2">
              ...
              <button onClick={() => { const val = prompt("보유 현금($) 입력", totalCashUSD.toString()); if(val) setTotalCashUSD(Number(val)); }} className="bg-white text-slate-900 px-4 py-2 rounded-full text-[10px] font-black uppercase">Cash 수정</button>
            </div>
          </div>
          <div className="grid grid-cols-6 text-[8px] font-black text-slate-400 border-b pb-2 mb-2 text-center uppercase"><div>티커</div><div>비중</div><div>자산(k)</div><div>수량</div><div>평단</div><div className="text-right">현재가</div></div>
          {processedAssets.list.map((s, i) => (
            <div key={i} onClick={() => s.isStock && setSelectedStock(s)} className="grid grid-cols-6 h-12 items-center italic border-b border-slate-50 text-center transition-all">
      ...
      {/* 2. WATCH (종목 추가 및 색상 설정 추가) [cite: 2026-01-11] */}
      {activeTab === 'WATCH' && (
      ...
      {/* 3. GOAL (종목 추가 및 색상 설정 추가) [cite: 2026-01-11] */}
      {activeTab === 'GOAL' && (
        <div className="p-4 animate-in fade-in">
          <div className="flex justify-between items-center mb-6 mt-2"><h1 className="font-black text-2xl italic tracking-tighter uppercase text-blue-600">Goal</h1><button onClick={() => setActiveTab('MAIN')} className="p-2 bg-slate-100 rounded-xl"><X size={24}/></button></div>
      ...
      {/* 4. SETTING (티커 연동형 색상 설정) [cite: 2026-01-11] */}
      {activeTab === 'SETTING' && (
        <div className="p-4 animate-in fade-in">
          ...
          <div className="grid grid-cols-7 text-[7px] font-black text-slate-400 border-b pb-2 mb-2 text-center uppercase"><div>색상</div><div>티커</div><div>수량</div><div>평단</div><div>손절</div><div>익절</div><div></div></div>
          {stocks.map((s, i) => (
            <div key={i} className="grid grid-cols-7 gap-1 items-center bg-slate-50 p-2 rounded-xl mb-2 italic">
              ...
              <input type="number" value={s.tp} onChange={e => { const n = [...stocks]; n[i].tp = Number(e.target.value); setStocks(n); }} className="text-[9px] w-full text-center bg-blue-100 text-blue-600 p-1 rounded font-bold" />
      ...
      {/* 사이드바 메뉴 (Navigation) [cite: 2026-01-11] */}
      {isSidebarOpen && activeTab === 'MAIN' && (
        <div className="fixed inset-0 z-[60] flex animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-md" onClick={() => setIsSidebarOpen(false)} />
          <div className="relative w-[80%] bg-white flex flex-col h-full ml-auto animate-in slide-in-from-right duration-300 shadow-2xl">
            <div className="p-8 flex justify-between items-center border-b font-black text-2xl italic uppercase mt-4">Menu <button onClick={() => setIsSidebarOpen(false)} className="p-2 bg-slate-100 rounded-xl"><X size={28}/></button></div>
            <div className="p-8 space-y-6">
              <button onClick={() => {setActiveTab('GOAL'); setIsSidebarOpen(false);}} className="w-full p-8 bg-blue-600 text-white rounded-[35px] font-black text-left flex justify-between items-center shadow-lg text-lg uppercase">Goal Portfolio <ChevronRight/></button>
              <button onClick={() => {setActiveTab('WATCH'); setIsSidebarOpen(false);}} className="w-full p-8 bg-orange-500 text-white rounded-[35px] font-black text-left flex justify-between items-center shadow-lg text-lg uppercase">Watchlist <ChevronRight/></button>
              <button onClick={() => {setActiveTab('SETTING'); setIsSidebarOpen(false);}} className="w-full p-8 bg-slate-900 text-white rounded-[35px] font-black text-left flex justify-between items-center shadow-lg text-lg uppercase">Asset Setting <ChevronRight/></button>
            </div>
          </div>
        </div>
      )}

      {/* 종목별 SL/TP 상세 팝업 [cite: 2026-01-11] */}
      {selectedStock && (
        <div className="fixed inset-0 z-[100] flex items-end animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setSelectedStock(null)} />
          <div className="relative w-full bg-white rounded-t-[50px] p-12 animate-in slide-in-from-bottom duration-500 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-8 mb-12">
              <div className="p-8 bg-blue-50 rounded-[40px] border border-blue-100 text-center shadow-inner"><p className="text-[11px] font-black mb-2 uppercase text-blue-400 tracking-widest">익절 목표 (TP)</p><p className="text-3xl font-black text-blue-700">${fUSD(selectedStock.tp)}</p></div>
              <div className="p-8 bg-red-50 rounded-[40px] border border-red-100 text-center shadow-inner"><p className="text-[11px] font-black mb-2 uppercase text-red-400 tracking-widest">손절 라인 (SL)</p><p className="text-3xl font-black text-red-700">${fUSD(selectedStock.sl)}</p></div>
            </div>
            <button onClick={() => setSelectedStock(null)} className="w-full py-6 bg-slate-900 text-white rounded-[30px] font-black text-xl shadow-2xl uppercase hover:bg-black transition-all">Done</button>
          </div>
        </div>
      )}
    </div>
