...
export default function StockDashboard() {
  ...
  const [exchangeRate, setExchangeRate] = useState(1457.37); // 스크린샷 기준 환율 적용 [cite: 2026-01-11]
  const [isLoaded, setIsLoaded] = useState(false);

  // 1. 초기 데이터 (티커 중심 관리) [cite: 2026-01-11]
  ...
  // --- 숫자 처리 규칙 (내림 및 k단위) [cite: 2026-01-11] ---
  ...
  const processedAssets = useMemo(() => {
    ...
    const cashItem = { ticker: 'CASH', totalUSD: totalCashUSD, totalKRW: totalCashUSD * exchangeRate, isStock: false, quantity: totalCashUSD, color: '#000' };
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen relative font-sans text-slate-900 pb-20">
      ...
      {/* 1. 메인 자산 화면 [cite: 2026-01-11] */}
      {activeTab === 'MAIN' && (
        <div className="animate-in fade-in duration-500">
          <div className="p-4 flex justify-between items-center mt-2">
            <div>
              <h1 className="font-black text-2xl italic tracking-tighter uppercase">My Assets</h1>
              <p className="text-[10px] font-bold text-slate-400">LIVE ₩{exchangeRate}</p>
            </div>
          ...
          <div className="px-4 mb-6">
            <div className="bg-[#0f172a] text-white p-6 rounded-[35px] shadow-2xl">
              ...
              <div className="flex justify-between items-center mt-6 border-t border-white/10 pt-4 px-2">
                ...
                <button onClick={() => {
                  const val = prompt("입금할 달러($)를 입력하세요");
                  if (val) setTotalCashUSD(prev => prev + Number(val));
                }} className="bg-white text-slate-900 px-4 py-2 rounded-full text-[10px] font-black shadow-lg">+ 달러 입금</button>
              </div>
            </div>
          </div>
          <div className="px-4">
            ...
            <div className="space-y-1">
              {processedAssets.list.map((s, i) => (
                <div key={i} onClick={() => s.isStock && setSelectedStock(s)} className="grid grid-cols-6 h-12 items-center italic border-b border-slate-50 active:bg-slate-50 cursor-pointer">
                  <div className="font-black text-[12px] text-left" style={{ color: s.color }}>{s.ticker}</div>
                  <div className="text-center font-bold text-slate-500 text-[10px]">{s.ratio.toFixed(1)}%</div>
                  <div className="text-center text-slate-900 text-[11px] font-black">{fKRWk(s.totalKRW)}</div>
                  <div className="text-center text-slate-400 text-[9px] font-bold">{s.isStock ? fUSD(s.quantity) : '-'}</div>
                  <div className="text-center text-slate-400 text-[9px] font-bold">{s.isStock ? fUSD(s.avgPrice) : '-'}</div>
            ...
            <div className="h-44 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={processedAssets.list} dataKey="ratio" nameKey="ticker" innerRadius={45} outerRadius={65} paddingAngle={3} label={({ticker, ratio}: any) => `${ticker} ${ratio.toFixed(0)}%`} labelLine={false}>
      ...
      {/* 2. 관심 종목 (WATCHLIST) [cite: 2026-01-11] */}
      {activeTab === 'WATCH' && (
        <div className="p-4 animate-in fade-in">
          <div className="flex justify-between items-center mb-6 mt-2">
            <h1 className="font-black text-2xl italic tracking-tighter uppercase">Watch</h1>
            <button onClick={() => {setActiveTab('MAIN'); setIsSidebarOpen(false);}} className="p-2 bg-slate-100 rounded-xl"><X size={24}/></button>
          </div>
          <div className="grid grid-cols-4 text-[9px] font-black text-slate-400 border-b pb-2 mb-2 text-center uppercase">
            <div className="text-left">티커</div><div>현재가</div><div>1차 진입</div><div>강력 진입</div>
          </div>
          {watchlist.map((w, i) => (
            <div key={i} className="grid grid-cols-4 h-14 items-center border-b border-slate-50 italic">
              <div className="font-black text-[13px] text-left" style={{ color: w.color }}>{w.ticker}</div>
              <div className="text-center font-black text-[11px]">${fUSD(w.current)}</div>
              <div className="text-center font-bold text-[11px] text-orange-500">${fUSD(w.part)}</div>
              <div className="text-center font-bold text-[11px] text-red-500">${fUSD(w.full)}</div>
            </div>
          ))}
        </div>
      )}

      {/* 3. 목표 비중 (GOAL) [cite: 2026-01-11] */}
      {activeTab === 'GOAL' && (
        <div className="p-4 animate-in fade-in">
          <div className="flex justify-between items-center mb-6 mt-2">
            <h1 className="font-black text-2xl italic tracking-tighter uppercase">Goal</h1>
            <button onClick={() => {setActiveTab('MAIN'); setIsSidebarOpen(false);}} className="p-2 bg-slate-100 rounded-xl"><X size={24}/></button>
          </div>
          <div className="grid grid-cols-3 text-[9px] font-black text-slate-400 border-b pb-2 mb-2 text-center uppercase">
            <div className="text-left">티커</div><div>목표 비율(%)</div><div className="text-right">목표 금액($)</div>
          </div>
          ...
          <div className="h-44 mt-10">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={targetPortfolio} dataKey="ratio" nameKey="ticker" innerRadius={45} outerRadius={65} paddingAngle={3} label={({ticker, ratio}: any) => `${ticker} ${ratio}%`} labelLine={false}>
                  {targetPortfolio.map((t, i) => <Cell key={i} fill={stocks.find(s => s.ticker === t.ticker)?.color || '#cbd5e1'} stroke="none" />)}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      {/* 4. 자산 설정 (SETTING) [cite: 2026-01-11] */}
      {activeTab === 'SETTING' && (
        <div className="p-4 animate-in fade-in">
          <div className="flex justify-between items-center mb-6 mt-2">
            <h1 className="font-black text-2xl italic tracking-tighter uppercase">Setting</h1>
            <button onClick={() => {setActiveTab('MAIN'); setIsSidebarOpen(false);}} className="p-2 bg-slate-100 rounded-xl"><X size={24}/></button>
          </div>
          <div className="grid grid-cols-6 text-[8px] font-black text-slate-400 border-b pb-2 mb-2 text-center uppercase">
            <div>색상</div><div>티커</div><div>수량</div><div>평단</div><div>손절</div><div>익절</div>
          </div>
          <div className="space-y-2">
            {stocks.map((s, i) => (
              <div key={i} className="grid grid-cols-6 gap-1 items-center bg-slate-50 p-3 rounded-2xl">
        ...
        <div className="fixed inset-0 z-[60] flex">
          ...
          <div className="relative w-full bg-white flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 p-6 space-y-4">
              <button onClick={() => {setActiveTab('GOAL'); setIsSidebarOpen(false);}} className="w-full p-6 bg-slate-900 text-white rounded-[30px] font-black text-left flex justify-between items-center shadow-xl tracking-tighter">GOAL PORTFOLIO <ChevronRight/></button>
              <button onClick={() => {setActiveTab('WATCH'); setIsSidebarOpen(false);}} className="w-full p-6 bg-white border-2 rounded-[30px] font-black text-left flex justify-between items-center tracking-tighter">WATCHLIST <ChevronRight/></button>
              <button onClick={() => {setActiveTab('SETTING'); setIsSidebarOpen(false);}} className="w-full p-6 bg-white border-2 rounded-[30px] font-black text-left flex justify-between items-center tracking-tighter">ASSET SETTING <ChevronRight/></button>
            </div>
          </div>
        </div>
      )}

      {/* 상세 팝업 (SL/TP) [cite: 2026-01-11] */}
      {selectedStock && (
        <div className="fixed inset-0 z-[100] flex items-end">
          ...
          <div className="relative w-full bg-white rounded-t-[45px] p-10 animate-in slide-in-from-bottom duration-300 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-6 mb-10">
              <div className="p-6 bg-blue-50 rounded-[35px] border border-blue-100 text-center"><p className="text-[10px] font-black mb-1 uppercase text-blue-400 tracking-widest">익절 목표</p><p className="text-2xl font-black text-blue-700">${fUSD(selectedStock.tp)}</p></div>
              <div className="p-6 bg-red-50 rounded-[35px] border border-red-100 text-center"><p className="text-[10px] font-black mb-1 uppercase text-red-400 tracking-widest">손절 라인</p><p className="text-2xl font-black text-red-700">${fUSD(selectedStock.sl)}</p></div>
            </div>
            <button onClick={() => setSelectedStock(null)} className="w-full py-5 bg-slate-900 text-white rounded-[25px] font-black text-lg">확인</button>
          </div>
        </div>
      )}
    </div>
